  ...
  const handleRoleSelect = async (role) => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        navigate("/verify-otp", {
          state: { email: signupData.email, message: response.data?.message },
        });
      } else {
      ...
      if (!error.response) {
        ...
        errorMessage =
          "Backend server unreachable. Please Sync your server URL or check your connection.";
      } else {
        errorMessage = error.response.data?.message || "Registration failed";
      }
      if (errorMessage === "Email already registered") {
        alert(
          "This email is already associated with an active account. Please sign in instead.",
        );
      } else {
        alert(errorMessage);
      }
    ...
    <div className="min-h-screen bg-[#0C0A09] flex items-center justify-center p-4 font-sans selection:bg-orange-600/30">
      <div className="max-w-2xl w-full bg-[#1C1917] rounded-2xl p-6 border border-stone-800/60 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden group">
        ...
        >
          ...
          <span className="text-xs font-bold tracking-tight ml-2">Back</span>
        </button>
        <div className="text-center mb-6 relative z-10">
          <div className="inline-flex items-center gap-2 text-orange-600 text-xs font-bold tracking-tight mb-4 bg-orange-600/5 px-4 py-1.5 rounded-full border border-orange-600/10 shadow-sm">
            <Lock size={12} strokeWidth={3} /> Account Authorization
          </div>
          <h2 className="text-xl font-bold tracking-tight text-white mb-2 leading-none">
            Select Access Level
          </h2>
          <p className="text-stone-500 font-bold tracking-tight text-xs opacity-80">
            Choose your system permissions
          </p>
        </div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-orange-600 font-bold tracking-tight text-xs">
                    {role.subtitle}
                  </span>
                </div>
                <h3 className="text-lg font-bold tracking-tight text-white group-hover:text-orange-500 transition-colors duration-500">
                  {role.label}
                </h3>
                <p className="text-stone-500 text-xs font-medium leading-relaxed mt-2 group-hover:text-stone-400 transition-colors duration-500">
                  {role.desc}
                </p>
              </div>
              <div
                className={`flex items-center gap-3 transition-all duration-500 pt-3 border-t border-stone-800/50 ${isLoading === role.id ? "opacity-100 text-orange-600" : "opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 text-stone-500 group-hover:text-orange-600"}`}
              >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-stone-600">
              ...
              <p className="text-xs font-bold tracking-tight leading-none">
                ...
                <span className="text-stone-800 mx-2">|</span> v4.2.0{" "}
              </p>
            </div>
            <button
              type="button"
              ...
              className="px-4 py-2 bg-stone-900 border border-stone-800 rounded-xl text-xs text-stone-500 hover:text-orange-600 hover:border-orange-600/30 font-bold tracking-tight transition-all active:scale-95 shadow-sm"
            >
              {" "}
              ⚙ SYNC SERVER{" "}
            </button>
