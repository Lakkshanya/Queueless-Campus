...
const ServiceManagement = () => {
  ...
  const availableCounters = counters.filter((c) => {
    const isAssignedToOther = services.some(
      (s) => s.assignedCounter?._id === c._id && s._id !== editingId,
    );
    ...
    <div className="space-y-12 animate-in">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
        <div>
          <div className="flex items-center gap-3 text-orange-600 text-[10px] font-bold tracking-tight mb-4 bg-orange-600/5 w-fit px-4 py-1.5 rounded-full border border-orange-600/10 uppercase">
            <Settings size={12} strokeWidth={3} /> Service Management
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-white leading-none">
            Campus Services
          </h1>
          <p className="text-stone-500 text-[11px] font-bold tracking-tight mt-6 max-w-xl leading-relaxed">
            Configure departmental service parameters, token prefixes, and
            operational throughput for the campus network.
          </p>
        </div>
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <div className="relative flex-1 lg:flex-none transform transition-all focus-within:scale-105">
            ...
            <input
              ...
              className="w-full lg:w-64 bg-[#171412] border border-stone-800/60 py-3.5 pl-12 pr-5 rounded-xl outline-none text-[11px] font-bold tracking-tight text-white placeholder-stone-800 focus:border-orange-600/40 transition-all"
            />
          </div>
          <button
            onClick={() => {
            ...
            className="bg-orange-600 hover:bg-orange-500 text-white py-3.5 px-6 rounded-xl font-bold tracking-tight text-[11px] transition-all shadow-lg flex items-center gap-3 active:scale-95 group uppercase"
          >
            <Plus size={16} strokeWidth={3} /> Deploy Service
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredServices.map((service) => (
          <div
            key={service._id}
            className="group relative bg-[#171412] border border-stone-800/40 rounded-[2.5rem] p-8 transition-all duration-500 hover:border-orange-600/30 hover:translate-y-[-8px] shadow-2xl overflow-hidden"
          >
            ...
            <div className="space-y-2 mb-8 relative z-10">
              <div className="flex items-center gap-3">
                <h3 className="text-2xl font-bold tracking-tight text-white group-hover:text-orange-500 transition-colors duration-500">
                  {service.name}
                </h3>
                <span className="px-3 py-1 bg-orange-600/10 border border-orange-600/20 rounded-lg text-xs font-bold tracking-tight text-orange-600">
                  {service.prefix}
                </span>
              </div>
              <p className="text-xs font-bold tracking-tight text-stone-600 line-clamp-2 leading-relaxed h-10 italic">
            ...
            <div className="grid grid-cols-2 gap-4 relative z-10">
              <div className="bg-stone-900/40 border border-stone-800/40 p-5 rounded-3xl group-hover:border-orange-600/20 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${service.assignedCounter ? "bg-emerald-500 animate-pulse" : "bg-red-500"}`}
                  />
                  <span className="text-xs font-bold tracking-tight text-white truncate">
                    {service.counterAssigned || "No Station"}
                  </span>
                </div>
              </div>
              <div className="bg-stone-900/40 border border-stone-800/40 p-5 rounded-3xl group-hover:border-orange-600/20 transition-colors">
            ...
            <div className="mt-6 pt-6 border-t border-stone-800/40 flex items-center justify-between relative z-10">
              <div className="flex items-center gap-4">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-bold tracking-tight text-stone-700 mt-1">
                    Token Limit
                  </span>
                </div>
                <div className="w-[1px] h-6 bg-stone-800" />
                <div className="flex flex-col">
                  <span className="text-[14px] font-bold tracking-tight text-orange-600 leading-none">
                    {service.estimatedTimePerStudent || 5}M
                  </span>
                  <span className="text-xs font-bold tracking-tight text-stone-700 mt-1">
                    Processing
                  </span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-stone-950/90 backdrop-blur-xl flex items-center justify-center p-6 z-[100] animate-in fade-in">
          <div className="bg-[#171412] w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-[3rem] border border-stone-800 shadow-[0_50px_100px_-20px_rgba(0,0,0,1)] relative p-6 md:p-10 custom-scrollbar">
            <button
            ...
            <div className="mb-12">
              ...
              <h2 className="text-2xl font-bold tracking-tight text-white">
                {editingId ? "Edit Service" : "Register Service"}
              </h2>
            </div>
            <form onSubmit={handleSubmit} className="space-y-10">
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <input
                    type="text"
                    value={formData.name}
                    placeholder="e.g. Finance Cell"
                    onChange={(e) =>
                      setFormData({ ...formData, name: e.target.value })
                    }
                    className="w-full bg-stone-900 border border-stone-800 p-6 rounded-2xl outline-none text-xs font-bold tracking-tight text-white focus:border-orange-600 transition shadow-inner"
                    required
                  />
                </div>
                <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <input
                  ...
                  placeholder="e.g. Floor 2, Room 402"
                  onChange={(e) =>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <select
                    ...
                    className="w-full bg-stone-900 border border-stone-800 p-6 rounded-2xl outline-none text-xs font-bold tracking-tight text-white focus:border-orange-600 transition appearance-none shadow-inner"
                  >
                    ...
                    {availableCounters
                      ...
                      .map((c) => (
                        <option key={c._id} value={c._id}>
                          Station {c.number.toString().padStart(2, "0")} (
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-6">
                  <label className="block text-xs font-bold tracking-tight text-stone-600 ml-2 text-center">
                    Throughput: {formData.estimatedTimePerStudent} MIN/STUDENT
                  </label>
                  <input
                    type="range"
                    min="1"
                    max="30"
                    value={formData.estimatedTimePerStudent}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        estimatedTimePerStudent: e.target.value,
                      })
                    }
                ...
                <div className="space-y-6">
                  <label className="block text-xs font-bold tracking-tight text-stone-600 ml-2 text-center">
                    Batch Avg: {formData.estimatedTimePerToken} MIN/BATCH
                  </label>
                  <input
                    type="range"
                    min="1"
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        estimatedTimePerToken: e.target.value,
                      })
                    }
                    className="w-full h-1.5 bg-stone-800 rounded-full appearance-none cursor-pointer accent-orange-600"
                  />
              ...
              <div className="space-y-4">
                ...
                <textarea
                  ...
                  className="w-full bg-stone-900 border border-stone-800 p-6 rounded-2xl outline-none text-xs font-bold tracking-tight text-white focus:border-orange-600 transition h-32 resize-none shadow-inner leading-relaxed"
                />
              </div>
              <button
                type="submit"
                className="w-full bg-orange-600 hover:bg-orange-500 py-6 rounded-2xl font-bold tracking-tight text-xs text-white shadow-xl transition-all flex items-center justify-center gap-4 active:scale-95"
              >
