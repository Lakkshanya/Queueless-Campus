      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 gap-5">
          <div className="flex flex-col gap-6">
            <div>
              ...
              <h1 className="text-lg font-black uppercase tracking-tighter leading-none text-white">Live Monitoring</h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {counters.map((counter) => (
            <div key={counter._id} className="group bg-[#1C1917] rounded-2xl p-6 border border-stone-800/50 hover:border-[#9A3412]/30 transition-all shadow-xl relative overflow-hidden flex flex-col">
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="w-20 h-20 bg-stone-900 border border-stone-800 rounded-xl flex items-center justify-center text-[#FAFAF9] group-hover:text-[#9A3412] transition-colors shadow-inner">
                   <span className="text-xl font-black tracking-tighter">{(counter.number || 0).toString().padStart(2, '0')}</span>
                </div>
              ...
              <div className="space-y-8 mb-6 relative z-10 flex-1">
                <div className="bg-[#1F1D1B] p-5 rounded-2xl border border-stone-800/50 group-hover:border-[#9A3412]/20 transition-all">
                  ...
                  {counter.currentToken ? (
                    <div className="flex justify-between items-center">
                      <h2 className="text-lg font-black text-[#9A3412] tracking-tighter">{counter.currentToken.number}</h2>
                    ...
                    <div className="flex items-center gap-4 text-stone-800">
                       ...
                       <p className="text-lg font-black uppercase tracking-tighter opacity-30">Counter Idle</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center px-4">
                  ...
                  <div className="text-right">
                     ...
                     <p className="text-lg font-black text-[#FAFAF9] tracking-tighter">{counter.workload || 0}</p>
                  </div>
                </div>
              </div>
        ...
        {counters.length === 0 && (
          <div className="flex flex-col items-center justify-center py-48 opacity-20 group">
             ...
             <div className="text-center">
                ...
                <h2 className="text-xl font-black uppercase tracking-[0.4em] text-stone-800">Searching Network</h2>
        ...
        <div className="mt-20 bg-[#1C1917] p-6 rounded-3xl border border-stone-800/50 flex flex-col lg:flex-row items-center gap-6 relative overflow-hidden">
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
              <div className="space-y-2">
                 <p className="text-[9px] font-black uppercase tracking-[0.3em] text-stone-600">Total Counters</p>
                 <p className="text-lg font-black text-[#FAFAF9]">{counters.length}</p>
              </div>
              <div className="space-y-2">
                 <p className="text-[9px] font-black uppercase tracking-[0.3em] text-stone-600">Active Operators</p>
                 <p className="text-lg font-black text-[#9A3412]">{counters.filter(c => c.status === 'active').length}</p>
              </div>
              <div className="space-y-2">
                 <p className="text-[9px] font-black uppercase tracking-[0.3em] text-stone-600">Total Enqueued</p>
                 <p className="text-lg font-black text-[#FAFAF9]">{counters.reduce((acc, c) => acc + (c.workload || 0), 0)}</p>
              </div>
              <div className="space-y-2">
        ...
        {tokenStats && (
          <div className="mt-8 mb-8 bg-[#1C1917] p-6 rounded-3xl border border-stone-800/50 relative overflow-hidden">
             ...
             <div className="mb-10 relative z-10">
                 <h2 className="text-xl font-black uppercase tracking-tighter text-white">Token Analytics</h2>
             ...
             <div className="grid grid-cols-2 md:grid-cols-5 gap-5 mb-6 relative z-10">
                <div className="bg-stone-900/50 p-6 rounded-3xl border border-stone-800/50">
                    ...
                    <p className="text-xl font-black text-white">{tokenStats.totalTokens}</p>
                </div>
                <div className="bg-stone-900/50 p-6 rounded-3xl border border-stone-800/50">
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-stone-500 mb-2">Served</p>
                    <p className="text-xl font-black text-green-500">{tokenStats.servedTokens}</p>
                </div>
                <div className="bg-stone-900/50 p-6 rounded-3xl border border-stone-800/50">
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-stone-500 mb-2">Serving</p>
                    <p className="text-xl font-black text-[#9A3412]">{tokenStats.servingTokens}</p>
                </div>
                <div className="bg-stone-900/50 p-6 rounded-3xl border border-stone-800/50">
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-stone-500 mb-2">Waiting</p>
                    <p className="text-xl font-black text-blue-500">{tokenStats.waitingTokens}</p>
                </div>
                <div className="bg-stone-900/50 p-6 rounded-3xl border border-stone-800/50">
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-stone-500 mb-2">Cancelled</p>
                    <p className="text-xl font-black text-red-500">{tokenStats.missedTokens}</p>
                </div>
             </div>
                 ...
                 <table className="w-full text-left min-w-[800px]">
                     <thead>
                         <tr className="border-b border-stone-800/40 opacity-70">
                             ...
                             <th className="px-4 py-4 text-[9px] font-black text-stone-500 uppercase tracking-[0.2em]">Counter</th>
                     ...
                     <tbody className="divide-y divide-stone-800/20">
                         ...
                         {tokenStats.recentTokens.length === 0 && (
                             <tr>
                               <td colSpan="6" className="text-center py-5 text-[10px] text-stone-600 font-bold uppercase tracking-widest">No recent token activity</td>
                             </tr>
                         )}
                     </tbody>
                 </table>
